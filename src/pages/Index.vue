   ...
   <q-page class="flex column" :class="bgClass">
      ...
      <template v-else>
         <section class="col column text-center text-white">
            <div class="col text-h2 text-weight-thin">JuSoft<br />Weather</div>
...
export default defineComponent({
   ...
   setup() {
      ...
      const getWeatherBySearch = async () => {
         try {
            const endpoint = `${weatherApiBaseUrl}/weather?q=${searchText.value}&appid=${weatherApiKey}&units=metric `
      ...
      const getWeatherByCoords = async () => {
         try {
            const endpoint = `${weatherApiBaseUrl}/weather?lat=${lat.value}&lon=${lon.value}&appid=${weatherApiKey}&units=metric `
            const response = await axios.get(endpoint)
            weatherData.value = response.data
            console.log('weatherData', weatherData.value)
         } catch (error) {
...
.q-page
   background: #00B4DB
   background: -webkit-linear-gradient(to bottom, #9fccfa, #0974f1)
   background: linear-gradient(to bottom, #9fccfa, #0974f1)
...
.skyline
   ...
   //background: url("https://image.freepik.com/free-vector/silhouette-skyline-illustration_53876-80575.jpg") bottom
</style>
